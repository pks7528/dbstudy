...
INSERT INTO EMPLOYEE_TBL(EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY) VALUES(EMPLOYEE_SEQ.NEXTVAL, '구창민', 1, '과장', 'M', '95-05-01', 5000000); --날짜 입력할 때는 '-', '/' 사용
...
-- 2. 부서번호(DEPART)가 1인 부서의 근무하는 사원들의 급여(SALARY)를 500000원 증가시키시오.
UPDATE EMPLOYEE_TBL
...
-- 데이터 삭제하기
-- 1. 지역(LOCATION)이 '대구'인 부서를 삭제하시오. ('대구'에서 근무하는사원이 없으므로 문제 없이 삭제된다.)
...
DELETE FROM DEPARTMENT_TBL WHERE LOCATION = '서울';       -- ON DELETE SET NULL 사용해서 부서는 없어졌지만 사원정보는 남아있음 // ON DELETE CASCADE 사용하면 사원정보도 다 날라감 
COMMIT;
