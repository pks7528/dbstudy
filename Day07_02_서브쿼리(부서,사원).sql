...
SELECT EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY
  FROM EMPLOYEE_TBL
 WHERE POSITION = (SELECT POSITION              -- 단일행// WHERE절에서 사용한 EMP_NO가 PK이기 때문에// PK랑 똑같은걸 찾는 것은 하나밖에 안나옴// = <-단일행 연산자
                     FROM EMPLOYEE_TBL
                    WHERE EMP_NO = 1001);
...
SELECT EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY
 ...
 WHERE SALARY = (SELECT MAX(SALARY)
                   FROM EMPLOYEE_TBL); -- MAX때문에 단일행 서브쿼리// MAX는 언제나 최대값 1개만 반환
...
-- 6. 부서번호가 2인부서에 근무하는 사원들의 직급과 일치하는 사원을 조회하시오.
--SELECT 사원정보
--  FROM 사원
...
SELECT EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY
 ...
 WHERE POSITION IN (SELECT POSITION                      
                     FROM EMPLOYEE_TBL
                    WHERE DEPART = 2);                   -- 서브쿼리의 WHERE 절에서 사용한 칼럼이 PK나 UNIQUE 칼럼이 아니므로 다중 행 서브쿼리이다.
...
SELECT EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY
 ...
 WHERE DEPART IN (SELECT DEPT_NO
                    FROM DEPARTMENT_TBL
                   WHERE DEPT_NAME IN '영업부');   -- 서브쿼리의 WHERE절에서 사용한 DEPART_NAME 칼럼은 PK/UNIQUE가 아니므로 다중 행 서브쿼리이다.
...
SELECT DEPT_NO, DEPT_NAME, LOCATION
 ...
 WHERE DEPT_NO IN (SELECT DEPART
                     FROM EMPLOYEE_TBL
                    WHERE POSITION IN '과장');        -- WHERE절에서 사용한 DEPART_NAME 칼럼은 PK/UNIQUE가 아니므로 다중 행 서브쿼리이다.
...
SELECT EMP_NO, NAME
  FROM EMPLOYEE_TBL
 WHERE ROWID = 'AAAFFEAABAAALDZAAA';
...
SELECT EMP_NO, NAME
  FROM EMPLOYEE_TBL
 WHERE ROWNUM <= 2; -- ROWNUM이 1울 포함한 범위가 사용되므로 가능!
...
SELECT ROWNUM AS RN, EMP_NO, NAME
  FROM EMPLOYEE_TBL
 WHERE RN = 2;  -- 실행 순서가 안맞기 때문에 실행이 불가능하다.(별명을 사용할 수 없다.)
                -- 별명 지정을 WHERE보다 먼저 처리하면 해결된다.
                -- 별명을 지정하는 인라인 뷰를 사용하면 가장 먼저 별명이 지정므로 해결된다.
...
-- 2) ROW_NUMBER() 함수 사용하기
SELECT E.EMP_NO, E.NAME, E.DEPART, E.POSITION, E.GENDER, E.HIRE_DATE, E.SALARY
  FROM (SELECT ROW_NUMBER() OVER (ORDER BY HIRE_DATE) AS RN, EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY
          FROM EMPLOYEE_TBL) E
 WHERE E.RN = 3 OR E.RN = 4;
