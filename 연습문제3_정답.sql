...
-- 1. 제품 테이블에서 제품명이 '책'인 제품의 카테고리를 '서적'으로 수정하시오. --123 DML 연습문제
UPDATE PRODUCTS
...
DELETE 
  FROM BUYS
 WHERE USER_ID = (SELECT USER_ID            --(사용자번호가 5번인 사용자의 USER_ID);  -- 서브쿼리로 해결해야 한다.
                    FROM USERS
                   WHERE USER_NO = 5);
...
-- 6. 제품을 구매한 사용자의 아이디별 구매횟수를 조회하시오.    // 아이디별 구매횟수 ->>>>>> 동일한 아이디는 하나로 모은다 ->>> GROUP BY 사용// 구매횟수 ->>> COUNT
-- 아이디  구매횟수                                                                            
-- KHD     3
-- PSH     3
-- KYM     1
...
SELECT USER_ID AS 아이디, COUNT(*) AS 구매횟수
  FROM BUYS
 GROUP BY USER_ID;                                          -- 셀렉트절에서 아이디를 조회하고 싶다.. GROUP BY는 아이디로 정해졌다


-- 7. 제품을 구매한 이력이 있는 고객의 아이디, 고객명, 구매횟수, 총구매액을 조회하시오. -- 테이블 2개 필요,, 조인, 그룹핑 같이 해야함 
-- 아이디  고객명  구매횟수  총구매액         
-- LHJ     이휘재  2         80
-- KYM     김용만  1         200
-- KHD     강호동  3         1210
...
SELECT U.USER_ID AS 아이디, U.USER_NAME AS 고객명, COUNT(*) AS 구매횟수, SUM(P.PROD_PRICE * B.BUY_AMOUNT) AS 총구매액
...
-- 9. 카테고리가 '전자'인 제품을 구매한 고객의 고객명, 제품명, 구매액을 조회하시오. -- 테이블 3개 이너조인으로
-- 고객명  제품명  구매액
-- 강호동  노트북  1000
-- 김용만  모니터  200
-- 박수홍  메모리  800
...
-- 11. 모든 제품의 제품명과 판매횟수를 조회하시오. -- 외부조인
...
-- 12. 어떤 고객이 어떤 제품을 구매했는지 조회하시오.   --구매이력도 없는 ->>> OUTER 조인
