/*
    ...
    1. 드라이브(DRIVE) 테이블
       ...
       3) 행(ROW)의 개수가 일반적으로 적고, PK를 조인 조건으로 사용하기 때문에 인덱스(INDEX) 활용이 가능하다. -- 풀스캔--인덱스를 못탐,, 인덱스를 탐--데이터가 어디에 있는지 기록을 남겨놓는 것을 의미,, 인덱스를 활용하면 어디있는지 알기 때문에 속도가 빠르다
    2. 드리븐(DRIVEN) 테이블
...
DELETE FROM EMPLOYEE_TBL WHERE NAME = '김성실' -- 인덱스를 타지 않는 일반 칼럼은 느리게 동작한다. // 김성실이 어디 저장되어 있는지 모르기 때문에 풀스캔
COMMIT; 

-- 외래키 제약조건의 활성화(다시 시작) -- 미완성
-- 제약조건이름 : FK_EMP_DEPT
ALTER TABLE EMPLOYEE_TBL
