...
/*
    오라클의 데이터 타입
    1. CHAR(size)    : 고정 길이 문자 타입(size : 1 ~ 2000 바이트) ex) 주민등록번호
    2. VARCHAR2(size) : 가변 길이 문자 타입(size : 1 ~ 4000 바이트) ex) 집주소
    3. DATE          : 날짜/시간 타입
    4. TIMESTAMP     : 날짜/시간 타입(좀 더 정밀)
    5. NUMBER(p,s)   : 정밀도(p), 스케일(s)로 표현하는 숫자 타입
        1) 정밀도 : 정부와 소수부를 모두 포함하는 전체 유효 숫자가 몇 개인가?
        2) 스케일 : 소수부의 전체 유효 숫자가 몇 개인가?
        예시)
            (1) NUMBER      : 최대 38자리 숫자(22바이트)
            (2) NUMBER(3)   : 정수부가 3자리(최대 999)
            (3) NUMBER(5,2) : 최대 전체 5자리, 소수부 2자리(123.45)
            (4) NUMBER(2,2) : 1미만의 소수부 2자리인 실수(0.15) - 정수부의 0은 유효 자리가 아님
*/
