...
-- 프로시저3 정의(만들기) : 사원 번호를 전달하면 해당 사원의 FIRST_NAME, LAST_NAME, SALARY를 조회하는 프로시저
-- 입력 파라미터
...
DECLARE 
    ...
    LNAME EMPLOYEES.LAST_NAME%TYPE;
BEGIN
    PROC4(FNAME, LNAME);    -- PLSQL 내부에서 프로시절르 호출할 때는 EXECUTE를 생략해야 한다.
    DBMS_OUTPUT.PUT_LINE(FNAME || ',' || LNAME);
END;

-- 프로시저5. 사원번호 전달하면, 해당 사원의 FIRST_NAME이 출력 파라미터 FNAME에 저장되도록 
...
ALTER TABLE BUY_TBL
    ADD CONSTRAINT FK_BUY_CUST FOREIGN KEY(C_NO) 
        REFERENCES CUST_TBL(C_NO) ON DELETE CASCADE;
ALTER TABLE BUY_TBL
    ADD CONSTRAINT FK_BUY_PROD FOREIGN KEY(P_CODE)
        REFERENCES PROD_TBL(P_CODE) ON DELETE SET NULL;

CREATE SEQUENCE BUY_SEQ
    NOCACHE;    -- CACHE는 속도향상을 위해 번호를 뽑아 메모리에 저장,, 미리 뽑아놓은 경우 메모리에서 날아가는 경우가 있으므로 CACHE는 안함
...
--1) 고객번호 1, 제품코드 1000, 구매수량 10
EXECUTE BUY_PROC(1, 1000, 10);  

--2) 고객번호 2, 제품코드 1001, 구매수량 5
BEGIN
    BUY_PROC(2, 1001, 5);
END;
