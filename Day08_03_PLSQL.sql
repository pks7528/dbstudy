...
/*
    ...
    3. 복사하는 쿼리
        ...
        2) 데이터를 제외하고 구조만 복사하기(칼럼만 복사하기)
            CREATE TABLE 테이블 AS(SELECT 칼럼 FROM 테이블 WHERE 1=2);      -- 1=2는 FALSE 만족하는 데이터없으므로 칼럼(구조)만 복사
*/

-- HR 계정의 EMPLOYEES 테이블을 GJD61 계정으로 복사해서 기초 데이터로 사용하기
...
-- EMPLOYEE_ID가150인 사원의 COMMISSION_PCT가 0이면 '커미션없음', 아니면 실제 커미션(COMMISSION_PCT * SALARY)을 출력하시오.
DECLARE
    EMP_ID EMPLOYEES.EMPLOYEE_ID%TYPE;
...
-- 사원번호 100~206인 사원들의 FIRST_NAME, LAST_NAME을 조회하시오.
DECLARE
    EMP_ID EMPLOYEES.EMPLOYEE_ID%TYPE;
    FNAME EMPLOYEES.FIRST_NAME%TYPE;
    LNAME EMPLOYEES.LAST_NAME%TYPE;
...
-- 사원번호가 100 ~ 206인 사원들의 평균 연봉을 출력하시오.
-- 연봉 평균 = 연봉 합 / 사원 수
DECLARE
    EMP_ID EMPLOYEES.EMPLOYEE_ID%TYPE;
    SAL EMPLOYEES.SALARY%TYPE;
...
-- 3) DEPART_ID가 50이면 행 변수에 저장된 내용을 DEPT50 테이블로 INSERT
...
-- 1부터 3의 배수를 제외한 정수 값을 누적하시오. 누적 값이 100을 초과하면 그만 누적하고 어디까지 누적했는지 출력하시오. -- CONTINUE 이용
DECLARE
    N NUMBER;
    TOTAL NUMBER;
    MODULAR NUMBER(1);
